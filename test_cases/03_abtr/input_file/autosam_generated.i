...
# ABTR 5-channel parallel bundle network (generated by AutoSAM)
# Source inputs:
# - intermediate_ABTR.yaml (geometry, areas, Dh, BCs, initial conditions, heat-structure widths)
# - model_specifications.xlsx (power fractions, friction factors, HTC, HT area density, branch form losses)
#
# ASSUMPTIONS (explicit):
# A1) EOS: using SAM built-in sodium EOS => PBSodiumEquationOfState (intermediate yaml had coolant=sodium but sam_eos_type was null).
# A2) Gravity: g = '0 0 -9.8' (not provided in intermediate yaml).
# A3) n_elems for all 1-D fluid components: 4 for CH1-CH5, 4 for Pipe2, 2 for Pipe1.
# A4) elem_number_of_hs (radial mesh counts) not provided; assumed:
#     - CH1-CH4: '2 1 1' for (fuel gap clad)
#     - CH5:    '2 1'   for (fuel clad)
# A5) Total power was absent from the case PDF; the analyst supplied the 250 MW
#     reference-case value at the intermediate-review stage.
# A6) Branch reference Area not provided; set to Pipe1/Pipe2 area = 0.44934 m^2.
# A7) Pipe1/Pipe2 friction factor not provided; set f = 0.02 (same order as channels).
#
...
[Components]
  ...
  [reactor]
    type = ReactorPower
    initial_power = 250e6  # W; supplied at analyst review from the reference case
  []
...
[Executioner]
  ...
  start_time = 0.0
  num_steps = 2000 # The number of timesteps in a transient run
  end_time = 100.
